...
int main()
{
    ...
    int filename_receive_len = 
        recvfrom(server_sock, filename_buffer, 
					FILENAME_MAXLEN, 0, 
                    &client_addr, 
                    &socket_addr_len);
	...
	if (fp == NULL)
    {
        ...
        sendto(server_sock, file_not_exist, 
                MAXLEN, 0, &client_addr, 
                socket_addr_len);
		fclose(fp);
	...
	while ((is_transferred==false)
			&&(file_content_len = 
                fread(file_download_buffer,
                    1,MAXLEN,fp) 
                > 0))
    {
		printf("transferring content:\n%s\n",file_download_buffer);
        sendto(server_sock, file_download_buffer,
                 MAXLEN, 0, &client_addr, 
                 socket_addr_len);
